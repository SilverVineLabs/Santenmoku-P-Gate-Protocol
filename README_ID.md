...
```mermaid
graph LR
    S1[Langkah 1: Kepercayaan Buta] --> S2[Langkah 2: Gesekan Kognitif]
    S2 --> S3[Langkah 3: Arsitektur Pagar Pengaman]
    S3 --> S4[Langkah 4: Eksekusi Terkendali]
    S4 --> S5[Langkah 5: Penguasaan Sumber Terbuka]
    S5 -.-> S1
```
...
```mermaid
flowchart TD
    subgraph S1 ["00-unsorted/ — Percikan Inspirasi Manusia"]
        A1["Intuisi Manusia & Wawasan Hidup<br/>(Pikiran mentah, kilatan inspirasi, bahan bakar jiwa)"]
        A2["Karakteristik: 100% Mentah & Belum Diproses"]
    end

    subgraph S2 ["Intersepsi Penerjemahan Strategis"]
        B1["Penerjemahan Strategis & Ekstraksi Prinsip Pertama"]
    end

    subgraph S3 ["01-daily-todos/ — Reduksi Dimensional & Matriks Aksi"]
        C1["🟢 Lapisan Aksi (Sepenuhnya Dapat Dikendalikan)<br/>TODAY_FOCUS.md"]
        C2["🟡 Lapisan Keyakinan (Strategi & Pola Pikir)<br/>STRATEGIC_BELIEFS.md"]
        C3["🔴 Lapisan Fakta (Verifikasi & Kebenaran Dasar)<br/>GROUND_TRUTH_CHECK.md"]
    end

    S1 -->|Ekstraksi| B1
    B1 -->|Reduksi Dimensional| S3
```
